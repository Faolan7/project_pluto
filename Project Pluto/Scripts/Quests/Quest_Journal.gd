...
# THIS WILL BE PLACED IN THE PLAYER AS A CHILD FOR TESTING PURPOSES
...
signal display_rewards_on_screen(xp, gold, items)
...
func add_quest(passed_npc: String, passed_quest_name: String, enemy_ID: int, num_of_enemies: int, rewards: Array) -> void:
...
# 		if yes:	We would emit a signal that says that the log is full
		if(current_number_of_quests == MAX_NUMBER_OF_QUESTS):
			...
			quest_scene.set_quest(passed_npc, passed_quest_name, enemy_ID, num_of_enemies, rewards)
...
func remove_quest(quest: Node) -> void:	
	...
	emit_signal("display_rewards_on_screen", quest.reward.get_xp(), quest.reward.get_gold(), quest.reward.get_items())
#	disperse_rewards(quest)
	quest.reward.quest_completed()


#func disperse_rewards(quest: Node) -> void:
#	print(quest.reward.get_xp())
#	print(quest.reward.get_gold())
#	print(quest.reward.get_items())
